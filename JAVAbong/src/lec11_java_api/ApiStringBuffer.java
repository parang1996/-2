...
public class ApiStringBuffer {
	public static void main(String[] args) {
		// 문자열하면 ? = String
		// String을 계속 수정하게되면 힙 메모리 영역에 계속 새로운 String 객체가 만들어지고
		// 그로인해 가비지컬렉터가 할 일이 많아진다.
		
		// 1부터 10까지의 숫자를 문자열로 다 더하면?\
		String strVal = "";
		for(int i=1; i <= 10; i++) {
			strVal += i;		// 매 순간 새로운 메모리 영역을 할당 받으며 기존 메모리 영역에 남아있는 값은 가비지 컬렉터의 의해 삭제된다 (비효율)
		}
		...
		// StringBuffer 가 가진 문자열을 String으로 변환
		...
		// 동기화란 ?
		// 멀티쓰레드 환경에서 해당 객체가 모든 쓰레드에서 안정적으로 사용 가능하도록 하는 것.
		
		// 동기화 미지원
		// 쓰레드 a,b,c 에서 strBuild를 사용할 때, 쓰레드 a에서
		// strBuild를 바꾼 경우 b,c 는 모른다
		// 단일 쓰레드를 이용한다면 좋은 성능
